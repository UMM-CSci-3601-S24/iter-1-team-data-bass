...
export class AddHuntComponent {
  ...
  getErrorMessage(title: keyof typeof this.addHuntValidationMessages): string {
    for(const {type, message} of this.addHuntValidationMessages[title]) {
      if (this.addHuntForm.get(title).hasError(type)) {
        return message;
      }
    }
