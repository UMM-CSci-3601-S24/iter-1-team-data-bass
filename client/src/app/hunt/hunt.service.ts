...
import { Hunt, UserRole } from './hunt';
...
export class UserService {
   ...
   */
  getUsers(filters?: { role?: UserRole; age?: number; company?: string }): Observable<Hunt[]> {
    ...
    return this.httpClient.get<Hunt[]>(this.userUrl, {
      params: httpParams,
    });
  }
   ...
   */
  getUserById(id: string): Observable<Hunt> {
    // The input to get could also be written as (this.userUrl + '/' + id)
    return this.httpClient.get<Hunt>(`${this.userUrl}/${id}`);
  }
   ...
   */
  filterUsers(users: Hunt[], filters: { name?: string; company?: string }): Hunt[] { // skipcq: JS-0105
  ...
  addUser(newUser: Partial<Hunt>): Observable<string> {
